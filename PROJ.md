...
- `rl/`：已有固定场景 RL 环境、DQN/Double DQN、PPO、MCTS baseline、DQN-guided MCTS、PPO-guided PUCT、MCTS->DQN 蒸馏、policy+value+PUCT、状态编码、训练与评估入口。
...
- reward 当前保持简洁并由 DQN/PPO/PUCT 共用：
  敌人掉血奖励 - 自身掉血惩罚 + 胜利/失败/timeout 终局信号。
...
```bash
.venv/bin/python -m rl.train_dqn_mcts_distill --config rl_configs/scenario5_big_jaw_worm.json --save-path rl_runs/dqn_mcts_distill.pt
```
...
```bash
.venv/bin/python -m rl.evaluate_dqn_mcts --config rl_configs/scenario5_big_jaw_worm.json --checkpoint rl_runs/dqn_scenario5_guardian.pt --simulations 30
```
...
```bash
.venv/bin/python -m rl.evaluate_ppo_puct --config rl_configs/scenario5_big_jaw_worm.json --checkpoint rl_runs/ppo_scenario5_guardian.pt --simulations 30
```
...
MCTS 是在线规划 baseline，不训练 checkpoint；默认用 50 次模拟/决策，可用 `--simulations` 调整。
...
这里是 AlphaZero 风格的固定战斗版本：PUCT 用 policy priors 和 value 做规划，训练目标是 MCTS visit policy + discounted return-to-go value，不做自对弈；早期叶子评估会混合启发式 value，避免随机网络主导搜索。
...
`rl_configs/scenario5_big_jaw_worm.json` 当前用于固定卡组实验；具体 enemy、ascension、deck 以 JSON 为准：

- enemy：`TheGuardian`
- ascension：`20`
- deck：包含 `Bludgeon`，当前 encoder observation size 为 `217`
- reward：敌人掉血、自身掉血和胜负终局信号
- training：默认 `episodes=1500`
...
- 当前已知表现排序：`DQN-guided MCTS` 最强但推理慢；`MCTS->DQN` v1 行为克隆蒸馏能提升纯 DQN；直接用 `DQN-guided MCTS` 做 v2 自举蒸馏目前会退化，暂不推荐。
- `MCTS->DQN` 蒸馏默认是行为克隆，只学习 teacher 最终动作；后续更值得做 root action value / visit distribution 蒸馏。
- `PPO-guided PUCT` 只是评估时搜索增强，PPO critic 与 PUCT 搜索策略存在 policy mismatch；模拟次数过大可能退化。
- `MCTS` / `DQN-guided MCTS` / 蒸馏 teacher 都很慢，主要成本来自 `simulations * rollout_depth * env deepcopy/step`。
