...
- RL 环境：支持固定动作空间、状态编码、DQN/Double DQN、PPO、MCTS baseline、DQN-guided MCTS、PPO-guided PUCT、policy+value+PUCT。
...
- `MCTS -> DQN distillation` 会在训练时用 MCTS 生成 expert action，让 DQN 通过行为克隆压缩搜索策略。
- `DQN-guided MCTS` 只在评估时加载 DQN checkpoint，用 DQN greedy rollout 和 `max Q` leaf value 做在线搜索增强。
- `PPO-guided PUCT` 只在评估时加载 PPO checkpoint 做在线搜索增强，不会更新 PPO 参数。
